...
namespace FlowerShop.Models
{
	public class ShoppingCart
	{
		...
		public bool AddToCart(Flower flower, int amount)
		{
			if(flower.InStock == 0 || amount == 0)
			{
				return false;
			}
			
			var shoppingCartItem = _context.ShoppingCartItems.SingleOrDefault(
				s => s.Flower.Id == flower.Id && s.ShoppingCartId == Id);
			...
			if (shoppingCartItem == null)
			{
                ...
                shoppingCartItem = new ShoppingCartItem
                {
                    ShoppingCartId = Id,
                    Flower = flower,
                    Amount = Math.Min(flower.InStock, amount)
				};
				_context.ShoppingCartItems.Add(shoppingCartItem);
			}
			else
			{
                if(flower.InStock - shoppingCartItem.Amount - amount >= 0)
                {
                    shoppingCartItem.Amount +=  amount;
                }
                else
                {
					shoppingCartItem.Amount += (flower.InStock - shoppingCartItem.Amount);
                    isValidAmount = false;
                }
            }
